...
<head>
    ...
    <title>@yield('title', 'My Account') — Styledinee</title>
...
<nav class="c-nav">
    ...
    @auth('customer_web')
    <ul class="c-nav__links" id="cNavLinks">
        <li><a href="{{ route('account.dashboard') }}" class="{{ request()->routeIs('account.dashboard') ? 'active' : '' }}">Dashboard</a></li>
        <li><a href="{{ route('account.orders') }}"    class="{{ request()->routeIs('account.orders')    ? 'active' : '' }}">My Orders</a></li>
        <li><a href="{{ route('account.wallet') }}"    class="{{ request()->routeIs('account.wallet')    ? 'active' : '' }}">Wallet</a></li>
        <li><a href="{{ route('account.profile') }}"   class="{{ request()->routeIs('account.profile')   ? 'active' : '' }}">Profile</a></li>
    </ul>

    <form method="POST" action="{{ route('account.logout') }}" class="c-nav__logout-form">
        @csrf
        <button type="submit" class="c-nav__logout">Log out</button>
    </form>
    @endauth

    <button class="c-nav__hamburger" id="cHamburger" aria-label="Menu">
...
<script>
    const cHamburger = document.getElementById('cHamburger');
    const cNavLinks  = document.getElementById('cNavLinks');
    if (cHamburger && cNavLinks) {
        cHamburger.addEventListener('click', () => cNavLinks.classList.toggle('open'));
    }
</script>
